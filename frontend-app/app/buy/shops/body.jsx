...
const Body = () => {
	...
	const [selectedShop, setSelectedShop] = useState("All Types")
	...
	const shopOptions = ["All Types", "Rental Shop", "Shopping Mall Unit", "Plaza", "Showroom"];
	...
	return (
		<div>
			<div className="mb-10">
				<h1 className="text-5xl text-black font-bold pb-3">Shops for sales in lagos</h1>
				<p className="text-md text-gray-400">Find shops for sale near you.</p>
			</div>
			{/* Filter Section */}
			<div className="flex flex-col md:flex-row items-stretch md:items-end justify-center border-2 px-6 py-6 md:px-2 md:py-4 rounded-md border-gray-300 gap-3 mb-8 w-full">
				...
				{/* Shop Filter */}
				<div className="relative w-full md:w-200" ref={shopRef}>
					<div className="text-gray-500 text-sm mb-1">Shop Type</div>
					<div className="flex flex-wrap gap-3">
						<button 
						...
						{isShopDropdownOpen && (
							<div className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg z-20 max-h-60 overflow-y-auto">
								...
								{/* Document options */}
								<div className="flex flex-col gap-1 p-2">
									{shopOptions.slice(1).map((option, index) => (
										<button
											key={index}
											onClick={() => handleShopSelect(option)}
											className={`px-3 py-2 text-sm text-left hover:bg-gray-100 rounded transition-colors ${selectedShop === option ? 'bg-blue-50 text-blue-600 font-medium' : ''}`}
										>
											{option}
										</button>
									))}
