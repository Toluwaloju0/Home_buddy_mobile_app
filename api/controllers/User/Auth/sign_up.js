...
const { UserService, TokenService, PasswordService, ValidationService, RefreshService } = services;
...
export default async function (request, response) {  
  try {
    ...
    await ValidationSservice.userValidator(userId)

    response.status(201).cookie('token', TokenService.createToken(userId), CookieOptions)
