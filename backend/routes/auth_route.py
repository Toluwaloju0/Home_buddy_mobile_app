...
@auth.post("/login")
def login(user_data: EmailLogin):
    """
    ...
    print(f"\n📧 Login request for: {user_data.email}")
    ...
    if get_user_response.status:
        ...
        print(f"✅ OTP sent to existing user")
        return response
    
    else:
        # New user - create account and send OTP
        print(f"📝 New user, creating account: {email}")
        ...
        user = User(
            email=email,
            first_name=email.split('@')[0],  # Use email prefix as temp first name
            is_verified=False,
